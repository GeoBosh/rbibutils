...
Export can be to plain text, bibtex, Latex, R code, html and Rd.  There is also a style,
...
for biblography items that are handled by \code{"bibentry"}.
...
wuertzetal
@ 
This modfied value is used below.
