...
register_JSSextra <- function(make_default = FALSE, reset = FALSE, parent_style = "JSS"){
    .Deprecated("",
      msg = 'No longer needed. to make JSSextra default, use tools::bibentry("JSSextra", .default = TRUE)\n')
...
bibstyle_JSSextraLongNames <- function(style = "JSSextraLongNames", reset = FALSE,
                                       make_default = FALSE){
        ## check if 'style' is registered; if so, returned it unless reset is TRUE
        JSSextra <- tools::bibstyle(style, .default = make_default)
        if(!is.null(JSSextra) && !reset)
            return(JSSextra)
